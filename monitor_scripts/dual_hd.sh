...
INT=`xrandr --current | sed 's/^\(.*\) connected.*$/\1/p;d' | grep -v ^VGA | head -n 1`
...
xrandr --output "${INT}" --auto --pos ${off_w}x${ext_h} --scale 1x1  --output "${EXT}" --auto --scale 2x2 --pos 0x0 --rotate normal --output HDMI1 --auto --scale 2x2 --pos 0x0
